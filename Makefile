...
release-local:
	 docker run --rm --privileged \
	  -v $$PWD:/go/src/github.com/oslokommune/goreleaser-test \
	  -v /var/run/docker.sock:/var/run/docker.sock \
	  -w /go/src/github.com/oslokommune/goreleaser-test \
	  ghcr.io/gythialy/golang-cross:v1.17.3-2 release --config=/go/src/github.com/oslokommune/goreleaser-test/.goreleaser-local.yml --snapshot --skip-publish --rm-dist
...
release:
	 docker run --rm --privileged \
	  ...
	  -w /go/src/github.com/oslokommune/goreleaser-test \
	  -e GITHUB_TOKEN \
	  ghcr.io/gythialy/golang-cross:v1.17.3-2 release --rm-dist --release-notes=$(RELEASE_NOTES) --config=/go/src/github.com/oslokommune/goreleaser-test/.goreleaser.yml
